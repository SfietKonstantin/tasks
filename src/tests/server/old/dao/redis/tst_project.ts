...
import {RedisProjectDao} from "../../../../../server/old/dao/redis/project"
import {RedisDaoBuilder} from "../../../../../server/old/dao/redis/builder"
import {RedisTestDataProvider} from "./testdataprovider"
import {project1, project2, project3, invalidProject} from "../../../../common/old/testdata"
...
describe("Redis DAO Project", () => {
    ...
    describe("getAllProjects", () => {
        ...
        xit("Should get only projects without corrupted body from the DB", () => {
            const projectKey = KeyFactory.createProjectKey(project1.identifier)
        ...
        xit("Should get an empty list of projects for a DB with corrupted keys", () => {
    ...
    describe("getProject", () => {
        xit("Should get a project from the DB", () => {
        ...
        xit("Should get an exception for a project with corrupted name", () => {
            const projectKey = KeyFactory.createProjectKey(project1.identifier)
        ...
        xit("Should get an exception for a project with corrupted description", () => {
            const projectKey = KeyFactory.createProjectKey(project1.identifier)
        ...
        xit("Should get an exception for a corrupted project", () => {
            const projectKey = KeyFactory.createProjectKey(project1.identifier)
            return RedisTestDataProvider.setValue(client, projectKey, "test").then(() => {
                return dao.getProject(project1.identifier)
            }).then(() => {
    ...
    describe("addProject", () => {
        xit("Should add a project in the DB", () => {
        ...
        xit("Should get an exception when adding an existing project in the DB", () => {
        ...
        xit("Should get an exception when adding a project in a DB with corrupted ids", () => {
            const projectIdsKey = KeyFactory.createGlobalProjectKey("ids")
