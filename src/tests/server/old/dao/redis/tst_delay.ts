...
import {RedisDelayDao} from "../../../../../server/old/dao/redis/delay"
import {RedisDaoBuilder} from "../../../../../server/old/dao/redis/builder"
import {RedisTestDataProvider} from "./testdataprovider"
import {project1, invalidProject, delayd1, delayd2, delayd3, invalidDelay} from "../../../../common/old/testdata"
...
describe("Redis DAO Delay", () => {
    ...
    describe("getDelay", () => {
        xit("Should get a delay from the DB", () => {
        ...
        xit("Should get an exception for a delay with corrupted name", () => {
        ...
        xit("Should get an exception for a delay with corrupted description", () => {
            const delayKey = KeyFactory.createDelayKey(project1.identifier, delayd1.identifier)
        ...
        xit("Should get an exception for a delay with corrupted date", () => {
            const delayKey = KeyFactory.createDelayKey(project1.identifier, delayd1.identifier)
        ...
        xit("Should get an exception for a corrupted delay", () => {
            const delayKey = KeyFactory.createDelayKey(project1.identifier, delayd1.identifier)
    ...
    describe("getProjectDelays", () => {
        xit("Should get a list of delays that belongs to a project from the DB", () => {
        ...
        xit("Should only get delays with body from the DB", () => {
            const delayKey = KeyFactory.createDelayKey(project1.identifier, delayd1.identifier)
        ...
        xit("Should only get delays without corrupted body from the DB", () => {
            const delayKey = KeyFactory.createDelayKey(project1.identifier, delayd1.identifier)
            return RedisTestDataProvider.setValue(client, delayKey, "test").then(() => {
                return dao.getProjectDelays(project1.identifier)
            }).then((delays: Array<DelayDefinition>) => {
    ...
    describe("addDelay", () => {
        xit("Should add a delay in the DB", () => {
        ...
        xit("Should get an exception when adding a delay in the DB for an invalid project", () => {
        ...
        xit("Should get an exception when adding an existing delay in the DB", () => {
        ...
        xit("Should get an exception when adding delay", () => {
